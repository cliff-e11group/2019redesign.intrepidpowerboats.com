	...
	class MUPR_AJAX extends Mass_users_password_reset {
		...
		public function send_reset_password_mail_action() {
			...
			if ( isset( $_POST['include'] ) || isset( $_POST['role'] ) || isset( $_POST['metakey'] ) || isset( $_POST['metavalue'] ) ) {
				...
				// If check is multisite installed OR not
				if ( is_multisite() ) {
					if ( is_super_admin() ) {
				...
				if (  !empty( $user_ids ) ) {
					...
					foreach( $user_ids as $user_id ) {
						 ...
						 * @var $user_id object
						 */
						do_action( 'mupr_after_reset', $user_id );
						// resetting password
						if ( $result == 1 ) {
							$sent_count++;
						} else {
		...
		public function mupr_save_options() {
			...
			if ( empty( $message ) ) {
				...
				$setting_options['message'] = stripslashes( wp_filter_post_kses ( $setting_options['message'] ) );
				...
				// Update mupr options
				$this->mupr_update_option( 'mupr_email_options', $setting_options );
				$message = array(
					'result'	=> 	1,
