...
if ( ! class_exists( 'Mass_users_password_reset' ) ) {
	...
	class Mass_users_password_reset extends WP_List_Table {
		...
		public $default_option = array(
			...
			'mupr_testmode_email'		=> '',
			'mupr_per_page'				=> 20,
			'mupr_reorder'				=> 'user_login,user_name,user_email,role,user_nicename,profile_picture,display_name',
			'mupr_user_row'				=> array( 'user_login', 'user_name', 'user_email', 'role' )
		);
		...
		public $steps = array(
			array(
				...
				'template'	=> MASS_USERS_PASSWORD_RESET_PRO_PLUGIN_DIR . 'admin/template/user-list.php'
			),
			array(
				'label'	=> 'Settings',
				'key'	=> 'settings',
				'icon'	=> 'dashicons-admin-generic',
				'template'	=> MASS_USERS_PASSWORD_RESET_PRO_PLUGIN_DIR . 'admin/template/settings.php'
			)
		);
		...
		function __construct() {
			...
			if ( is_multisite() && ! is_super_admin() ) {
				foreach ( get_super_admins() as $key => $value ) {
					$super_admin = get_user_by( 'login', $value );	
					$this->exclude[] = $super_admin->ID;
				}
			}
			// End
			// Set table rows
			$this->user_rows = array(
				'user_login'		=> __( 'Username', 'mass-users-password-reset-pro' ),
				'user_name'			=> __( 'Name', 'mass-users-password-reset-pro' ),
				'user_email'		=> __( 'Email', 'mass-users-password-reset-pro' ),
				'role'				=> __( 'Role', 'mass-users-password-reset-pro' ),
				'profile_picture' 	=> __( 'Profile Picture', 'mass-users-password-reset-pro' ),
				'user_nicename' 	=> __( 'Nickname', 'mass-users-password-reset-pro' ),
				'display_name' 		=> __( 'Display Name', 'mass-users-password-reset-pro' ),
			);
			// include in admin menu
			add_action( 'network_admin_menu', array( $this, 'mass_users_password_reset_menu' ) );
			...
			// Add new site
			add_action( 'wp_insert_site', array( $this, 'mass_users_password_reset_insert_site' ) );
		}
		...
		function mass_users_password_reset_menu() {
			add_submenu_page( 'users.php', 'Mass Users Password Reset', 'Mass Users Password Reset Pro',
				'manage_options', 'mass_users_password_reset_options',
		...
		private function replace_user_info_in_mail( $user_info, $message_body, $send_link = 'no' ) {
			...
			// Set custom password.
			$new_password = apply_filters( 'mupr_set_user_password', $new_password, $user_info );
		...
		function send_email_format( $user_id ) {
			$blog_id = ( int ) isset( $_REQUEST['site'] ) ? $_REQUEST['site'] : 1;
			// Options
			$email_option = $this->mupr_get_option( $blog_id );
			// Set default option using wp_parse_args function
			$custom_email = wp_parse_args( $email_option, $this->options );
			$current_site_option = $this->options;
			$user_info = get_userdata( $user_id );
			$email_content = $this->replace_user_info_in_mail( $user_info,$custom_email['message'], $custom_email['mupr_to_send_reset_link'] );

			if ( isset( $current_site_option['mupr_testmode'] ) && $current_site_option['mupr_testmode'] == true ) {
				$to = isset( $current_site_option['mupr_testmode_email'] ) && ! empty( $current_site_option['mupr_testmode_email'] ) ? $current_site_option['mupr_testmode_email'] : $user_info->user_email;
			} else {
				$to = $user_info->user_email;
			}

			$subject = sanitize_text_field( $custom_email['subject'] );
			$body = wpautop( $email_content, true );
			...
			if ( ! apply_filters( 'mupr_send_email', true ) ) {
				return true;
			}
		...
		/**
		 * Multisite sites filter
		 */
		private function mupr_user_sites_filter() {
			$filter = '';
			$current_site = ( int ) isset( $_REQUEST['site'] ) ? $_REQUEST['site'] : 1;
			if ( function_exists( 'get_sites' ) && class_exists( 'WP_Site_Query' ) ) {
				$sites = get_sites();
				foreach ( $sites as $site ) {
					$filter .='<option value="' . $site->blog_id . '" ' . selected( $site->blog_id, $current_site , false ) . '>' . $site->blogname . '</option>';
				}
			}
			return $filter;
		}
		...
		private function mupr_custom_field_filter() {
			...
			// Get metakey list
			$metakey_list = $this->wpdb->get_results( 'SELECT distinct meta_key FROM ' . $this->wpdb->base_prefix .'usermeta WHERE meta_value NOT LIKE "a:%:{%}" AND meta_key NOT IN ' . $exclude_metakeys, ARRAY_A );
			// Get user meta keys
			$user_metakey_query = array(
			...
			if ( is_multisite() ) {
				...
				$user_metakey_query['blog_id'] = $blog_id;
			}
			$users_list = new WP_User_Query( $user_metakey_query );
		...
		private function mupr_user_lists() {
			...
			if ( is_multisite() ) {
				...
				$all_users_query['blog_id'] = $blog_id;
				$user_query['blog_id'] = $blog_id;
			}
		 ...
		 * @return bool
		 */
		public function mupr_update_option( $option, $value ) {
			if ( is_multisite() ) {
				update_blog_option( '', $option, $value );
			} else {
		 ...
		 * @return array|obj
		 */
		public function mupr_get_option( $id = '', $option = 'mupr_email_options' ) {
			return is_multisite() ? get_blog_option( $id, $option ) : get_option( $option );
		}
		 ...
		 */
		private function mupr_get_admin_url( $path, $scheme = 'admin' ) {
			$admin_url = '';
			if ( is_multisite() ) {
				if ( is_admin() && get_current_screen()->is_network ) {
					$admin_url = network_admin_url( $path );

				} else {
					$admin_url = get_admin_url( '', $path );
				}
			} else {
		...
		/**
		 * Get sites using user ID
		 * @param int $user_id
		 * @return array sites
		 */
		private function mupr_get_sites_by_user( $user_id, $key = '' ) {
		 ...
		 * @param object $site
		 */
		public function mass_users_password_reset_insert_site( $site ) {
			$new_blog_id = $site->blog_id;
			...
			$custom_email_options = array(
				'mupr_to_send_reset_link'	=>	'no',
				'from_email'		=>	get_blog_option( $new_blog_id, 'admin_email' ),
				'from_name'			=> 	get_blog_option( $new_blog_id, 'blogname' ),
				'subject'			=>	'Reset Password',
				'message'			=>	$email_content
			);
			add_blog_option( $new_blog_id, 'mupr_email_options', $custom_email_options );
		}
