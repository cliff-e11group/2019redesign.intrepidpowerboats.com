			...
			<div class="mupr-mail-setting">
				<div class="mupr-mail-setting-left">
					<div class="mupr-form-wrap">
						...
						<div class="mupr-row">
							<div class="mupr-column half">
								<div class="mupr-sortable-group">
									<h3><?php _e( 'Users columns to show', 'mass-users-password-reset-pro' ); ?></h3>
									<input type="hidden" name="mupr_reorder" id="mupr_reorder" value="<?php echo $options['mupr_reorder']; ?>">
									<ul id="sortable">
										<?php
										$sortable_data = array_replace( array_flip( explode( ',', $options['mupr_reorder'] ) ), $this->user_rows );
										foreach( $sortable_data as $key => $rows ) : ?>
											<li class="ui-state-default mupr-list" id="<?php echo $key; ?>">
												<span class="mupr-drag-icon"></span><input type="checkbox" name="mupr_user_row[]" value="<?php echo esc_attr( $key ); ?>"<?php echo in_array( $key, $options['mupr_user_row'] ) ? ' checked' : ''; ?>><?php echo $rows; ?>
											</li>
										<?php endforeach; ?>
									</ul>
								</div>
...
%3$s : {NEW_PASSWORD}', __( 'Dear', 'mass-users-password-reset-pro' ), __( 'Your Password has been changed.', 'mass-users-password-reset-pro' ), __( 'Your new password is', 'mass-users-password-reset-pro' ) );
										?>
										...
										<?php
											...
											$content = ! empty( $options['message'] ) ? $options['message'] : $default_message_content;
											wp_editor( $content, 'mupr-user-notification', $settings );
										?>
									</div>
								</div>
							</div>
