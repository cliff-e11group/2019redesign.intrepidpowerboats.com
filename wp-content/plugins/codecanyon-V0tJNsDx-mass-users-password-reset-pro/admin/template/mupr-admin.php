<div class="mupr-wrap">
	<div class="spinner">
		<?php if ( ( ! isset( $_GET[ 'tab' ] ) ) || ( isset( $_GET[ 'tab' ] ) && 'user-list' === $_GET[ 'tab' ] ) ) : ?>
			<div class="mupr-cancel">
				<a href="javascript:;" class="button mupr-btn"><?php _e( 'Cancel', 'mass-users-password-reset-pro' ); ?></a>
			</div>
		<?php endif; ?>
	</div>
	<div class="wrap">
		...
		<div class="mupr-header">
			<div class="mupr-header-right">
				<div class="logo">
					<a href="<?php echo esc_url( 'https://codecanyon.net/item/mass-users-password-reset-pro/20809350' ); ?>" target="_blank"><img src="<?php echo MASS_USERS_PASSWORD_RESET_PRO_PLUGIN_URL; ?>assets/images/mass-user-password-pro-logo.png" alt="Mass User Password Reset"></a>
				</div>
			</div>
			<div class="mupr-header-left">
				<div class="nav-tab-wrapper mupr-tab">
					<?php
						...
						?>
						<a href="<?php echo esc_url( add_query_arg( array( 'page' => 'mass_users_password_reset_options', 'tab' => $tab_name ), $this->mupr_get_admin_url( 'users.php' ) ) ); ?>" class="nav-tab<?php echo esc_attr( $active_tab == $tab_name ? ' active' : '' ); ?>"><span class="dashicons<?php echo esc_attr( $tab_icon ); ?>"></span> <?php _e( isset( $step['label'] ) ? $step['label'] : '' ); ?></a>
					<?php endforeach; ?>
