	...
	protected function set_url( $connected_account, $endpoint_slug, $params ) {
		...
		if ( $account_type === 'personal' ) {
			...
			if ( $endpoint_slug === 'header' ) {
				...
				$url = 'https://graph.facebook.com/'.$params['hashtag_id'].'/top_media?user_id='.$connected_account['user_id'].'&fields=media_url,caption,id,media_type,comments_count,like_count,permalink,children{media_url,id,media_type,permalink}&limit='.$num.'&access_token='.sbi_maybe_clean( $connected_account['access_token'] );
			} elseif ( $endpoint_slug === 'hashtags_recent' ) {
				$url = 'https://graph.facebook.com/'.$params['hashtag_id'].'/recent_media?user_id='.$connected_account['user_id'].'&fields=media_url,caption,id,media_type,comments_count,like_count,permalink,children{media_url,id,media_type,permalink}&limit='.$num.'&access_token='.sbi_maybe_clean( $connected_account['access_token'] );
