    ...
    {   
        ...
        return preg_replace_callback($pattern,function( $matches ) use ( $autoData, $elm_position ){
            ...
            if ( (preg_match('/<iframe.*(src=\"(.*)\").*>.*<\/iframe>/', $match, $element_match)) ||(preg_match('/<object.*(src=\"(.*)\").*>.*<\/object >/', $match, $element_match)) || (preg_match('/<embed.*(src=\"(.*)\").*>/', $match, $element_match)) || (preg_match('/<img.*(src=\"(.*)\").*>/', $match, $element_match)) ) {
                ...
                $element_modded_src = str_replace('src=',$wt_cli_replace.' data-cli-placeholder="'.$placeholder_text.'" data-cli-src=',$element_src);
                ...
                if (preg_match("/type=/", $match)) {
                    ...
                    if(!empty($output_array)) {   
                        $match = str_replace($output_array[1], 'type="'.$script_type.'"'.' '.$wt_cli_replace, $match);
                    }
