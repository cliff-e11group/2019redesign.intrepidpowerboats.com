...
class Cookie_Law_Info {
	...
	*/
	...
	public static function get_settings()
	{
		...
		if(isset($_GET['cli_bypass']) && get_option('CLI_BYPASS')==1)
		{
	...
	public static function get_cookie_by_category()
	{
		...
		$terms = $wpdb->get_results("SELECT slug FROM $wpdb->terms as t 
		INNER JOIN $wpdb->term_taxonomy AS tt ON (t.term_id = tt.term_id)
		WHERE tt.taxonomy = 'cookielawinfo-category';
		");
		...
		{	
			...
			AND t.slug = %s
			ORDER BY p.post_date DESC",$key,$viewed_cookie,$category_slug));
			...
			{	
				$non_necessary_cookies[$category_slug]=$wt_cli_cookies;
			}
